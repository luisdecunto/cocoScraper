"""
Vital brand similarity analysis.

Reads all unique brands from the postprocessed vital_products.txt and runs
fuzzy similarity clustering to surface typos and near-duplicates.

Usage:
    python exports/vital_brand_similarity.py [--threshold 82]

Output:
    exports/vital_brand_groups.txt  — grouped near-duplicate brands
    exports/vital_brands_unique.txt — sorted unique brand list
"""
...
def load_brands(products_path: Path) -> list[str]:
    """Parse the Marca column from vital_products.txt."""
...
def main(threshold: float) -> None:
    ...
    brands = load_brands(base / "vital_products.txt")
    print(f"Unique brands: {len(brands)}")

    unique_path = base / "vital_brands_unique.txt"
    ...
    groups_path = base / "vital_brand_groups.txt"
    lines = [
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Vital brand similarity analysis")
    parser.add_argument(
